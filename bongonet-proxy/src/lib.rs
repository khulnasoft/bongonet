...
//! # bongonet-proxy
//!
//! Programmable HTTP proxy built on top of [bongonet_core].
//!
//! # Features
...
//!
//! Then the service can be passed into [`http_proxy_service()`] for a [bongonet_core::server::Server] to
//! run it.
//!
...
use bongonet_cache::NoCacheReason;
use bongonet_core::apps::{HttpServerApp, HttpServerOptions};
use bongonet_core::connectors::{http::Connector, ConnectorOptions};
use bongonet_core::modules::http::compression::ResponseCompressionBuilder;
use bongonet_core::modules::http::{HttpModuleCtx, HttpModules};
use bongonet_core::protocols::http::client::HttpSession as ClientSession;
use bongonet_core::protocols::http::v1::client::HttpSession as HttpSessionV1;
use bongonet_core::protocols::http::HttpTask;
use bongonet_core::protocols::http::ServerSession as HttpSession;
use bongonet_core::protocols::http::SERVER_NAME;
use bongonet_core::protocols::Stream;
use bongonet_core::protocols::{Digest, UniqueID};
use bongonet_core::server::configuration::ServerConf;
use bongonet_core::server::ShutdownWatch;
...
use bongonet_cache::HttpCache;
use bongonet_core::protocols::http::compression::ResponseCompressionCtx;
...
error[E0391]: cycle detected when computing type of `proxy_cache::<impl at bongonet-proxy/src/proxy_cache.rs:7:1: 7:23>::proxy_cache::{opaque#0}`
   --> bongonet-proxy/src/proxy_cache.rs:13:10
    |
...
///
/// The returned [Service] can be hosted by a [bongonet_core::server::Server] directly.
pub fn http_proxy_service<SV>(conf: &Arc<ServerConf>, inner: SV) -> Service<HttpProxy<SV>>
where
    SV: ProxyHttp,
{
    http_proxy_service_with_name(conf, inner, "Bongonet HTTP Proxy Service")
}
