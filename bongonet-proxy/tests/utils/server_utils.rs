...
impl ProxyHttp for ExampleProxyCache {
    ...
    fn request_cache_filter(&self, session: &mut Session, _ctx: &mut Self::CTX) -> Result<()> {
        ...
        let eviction = session.req_header().headers.get("x-eviction").map(|_| {
            &*EVICTION_MANAGER as &'static (dyn bongonet_cache::eviction::EvictionManager + Sync)
        });
        let lock = session
            .req_header()
            .headers
...
fn test_main() {
    ...
    let opts: Vec<String> = vec![
        "bongonet-proxy".into(),
        "-c".into(),
        "tests/bongonet_conf.yaml".into(),
    ];
    let mut my_server = bongonet_core::server::Server::new(Some(Opt::parse_from(opts))).unwrap();
    my_server.bootstrap();
