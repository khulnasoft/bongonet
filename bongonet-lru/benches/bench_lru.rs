...
fn main() {
    ...
    let plru = bongonet_lru::Lru::<(), 10>::with_capacity(1000, 100);
    ...
    let elapsed = before.elapsed();
    println!(
        "bongonet lru promote total {elapsed:?}, {:?} avg per operation",
        elapsed / ITERATIONS as u32
    );
    ...
    let elapsed = before.elapsed();
    println!(
        "bongonet lru promote_top_10 total {elapsed:?}, {:?} avg per operation",
        elapsed / ITERATIONS as u32
    );
    ...
    for i in 0..THREADS {
        ...
        let handler = thread::spawn(move || {
            ...
            let elapsed = before.elapsed();
            println!(
                "bongonet lru promote total {elapsed:?}, {:?} avg per operation thread {i}",
                elapsed / ITERATIONS as u32
            );
        });
        handlers.push(handler);
    ...
    for i in 0..THREADS {
        ...
        let handler = thread::spawn(move || {
            ...
            let elapsed = before.elapsed();
            println!(
                "bongonet lru promote_top_10 total {elapsed:?}, {:?} avg per operation thread {i}",
                elapsed / ITERATIONS as u32
            );
        });
        handlers.push(handler);
