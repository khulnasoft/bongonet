...
use bongonet_timeout::timer::TimerManager;
...
fn bench_timer(tm: &TimerManager) {
    ...
    let elapsed = before.elapsed();
    println!(
        "bongonet timer create {:?} total, {:?} avg per iteration",
        elapsed,
        elapsed / LOOP_SIZE
    );
    ...
    let elapsed = before.elapsed();
    println!(
        "bongonet timer drop {:?} total, {:?} avg per iteration",
        elapsed,
        elapsed / LOOP_SIZE
    );
}
...
async fn main() {
    ...
    let elapsed = before.elapsed();
    println!(
        "bongonet timeout {:?} total, {:?} avg per iteration",
        elapsed,
        elapsed / LOOP_SIZE
    );
    ...
    let elapsed = before.elapsed();
    println!(
        "bongonet fast timeout {:?} total, {:?} avg per iteration",
        elapsed,
        elapsed / LOOP_SIZE
    );
    ...
    let tm = bongonet_timeout::timer::TimerManager::new();
    bench_timer(&tm);
    bench_tokio_timer();
