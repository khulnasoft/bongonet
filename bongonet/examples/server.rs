...
use bongonet::protocols::TcpKeepalive;
use bongonet::server::configuration::Opt;
use bongonet::server::{Server, ShutdownWatch};
use bongonet::services::background::{background_service, BackgroundService};
use bongonet::services::{listening::Service as ListeningService, Service};
...
use bongonet::tls::pkey::{PKey, Private};
use bongonet::tls::x509::X509;
struct DynamicCert {
    cert: X509,
    key: PKey<Private>,
}
...
#[async_trait]
impl bongonet::listeners::TlsAccept for DynamicCert {
    async fn certificate_callback(&self, ssl: &mut bongonet::tls::ssl::SslRef) {
        use bongonet::tls::ext;
...
pub fn main() {
    ...
    let mut options = bongonet::listeners::TcpSocketOptions::default();
    ...
    let mut tls_settings = bongonet::listeners::TlsSettings::with_callbacks(dynamic_cert).unwrap();
    // by default intermediate supports both TLS 1.2 and 1.3. We force to tls 1.2 just for the demo
    tls_settings
        .set_max_proto_version(Some(bongonet::tls::ssl::SslVersion::TLS1_2))
        .unwrap();
