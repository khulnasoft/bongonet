...
mod tests {
    ...
    fn test_send_receive_fds() {
        ...
        let child = thread::spawn(move || {
            ...
            let (fds, bytes) = get_fds_from("/tmp/bongonet_fds_receive.sock", &mut buf).unwrap();
            debug!("{:?}", fds);
            assert_eq!(1, fds.len());
            assert_eq!(32, bytes);
            assert_eq!(1, buf[0]);
        ...
        match send_fds_to(fds, &buf, "/tmp/bongonet_fds_receive.sock") {
            Ok(sent) => {
                assert!(sent > 0);
            }
            Err(e) => {
    ...
    fn test_serde_via_socket() {
        ...
        let child = thread::spawn(move || {
            let mut fds2 = Fds::new();
            fds2.get_from_sock("/tmp/bongonet_fds_receive2.sock")
                .unwrap();
            assert!(*fds2.get(&key1).unwrap() > 0);
            assert!(*fds2.get(&key2).unwrap() > 0);
        });

        fds.send_to_sock("/tmp/bongonet_fds_receive2.sock").unwrap();
        child.join().unwrap();
    }
}
