...
//!
//! A [BackgroundService] can be run as part of a Bongonet application to add supporting logic that
...
pub trait BackgroundService {
    /// This function is called when the bongonet server tries to start all the
...
impl<A> GenBackgroundService<A> {
    /// Generates a background service that can run in the bongonet runtime
